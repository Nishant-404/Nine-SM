          ...
          await audio_service.AudioService.init<_SpotiFLACAudioHandler>(
            ...
            config: const audio_service.AudioServiceConfig(
              androidNotificationChannelId:
                  'com.nisha.twentyfourplayer.playback',
...
// ─── Public: add track to queue ──────────────────────────────────────────
  void addToQueue(Track track) {
    ...
    final hasValidIndex = state.currentIndex >= 0 && state.currentIndex < state.queue.length;

    if (hasValidIndex) {
      // 1. Insert right after the currently playing song in the main queue
      final insertIndex = state.currentIndex + 1;
      newQueue.insert(insertIndex, item);

      // 2. The Shuffle Matrix Fix: Force it to play next even if shuffled
      if (state.shuffle) {
        // Shift all existing indices that got pushed down by our insert so the engine doesn't crash
        for (int i = 0; i < _shuffleOrder.length; i++) {
          if (_shuffleOrder[i] >= insertIndex) {
            _shuffleOrder[i]++;
          }
        }
        // Inject our new track exactly one spot ahead of the current playing song
        final nextShufflePos = _shufflePosition + 1;
        if (nextShufflePos <= _shuffleOrder.length) {
          _shuffleOrder.insert(nextShufflePos, insertIndex);
        } else {
          _shuffleOrder.add(insertIndex);
        }
      }
    } else {
      // Queue is empty, just add normally
      newQueue.add(item);
      if (state.shuffle) {
  ...
  Future<void> setBassHardware(double level) async {
    // The Sub-Bass Macro: We boost the bottom 3 bands (31Hz, 62Hz, 125Hz)
    final params = await _equalizer.parameters;
    final bands = params.bands;
    if (bands.length >= 3) {
      await bands[0].setGain(level * 15.0);
      await bands[1].setGain(level * 15.0);
      await bands[2].setGain(level * 10.0); // Slight taper to prevent muddiness
    }
  }

  Future<void> setTrebleHardware(double level) async {
    // The Treble Macro: We boost the top 3 bands (4k, 8k, 16k)
    final params = await _equalizer.parameters;
    final bands = params.bands;
    if (bands.length >= 10) {
      await bands[7].setGain(level * 15.0);
      await bands[8].setGain(level * 15.0);
      await bands[9].setGain(level * 15.0);
    }
  }

  Future<void> setEqBandHardware(int index, double level) async {
    final params = await _equalizer.parameters;
    final bands = params.bands;
    if (index < bands.length) {
      await bands[index].setGain(level * 15.0);
    }
  }
}
